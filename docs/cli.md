...
| `users.lock-root` | interactive only | yes | Runs in whichever direction the host is in, so one row covers both. Where root can log in, expires it so no method admits it, including the provider's rescue console. Takes no arguments: it scans every account on the host and refuses unless at least one of them can escalate *and* authenticate — by an authorised key or by a usable password. The confirmation lists each account that qualifies, with the credential it gets in by. Refuses unless at least one account can still get in — and asks `sudo` itself rather than trusting group membership: an administrator who removes the group's rule from `sudoers` leaves an account that reads back as a member and can escalate nothing. Asked by exit code (`sudo -l -U <user> /bin/true`), never by reading sudo's wording. Only a refusal is believed: openSUSE grants every account everything through `Defaults targetpw`, using the very root password this task removes. Where root is *already* locked, this lifts the expiry instead: no scan and no refusal, since restoring access can strand nobody, and no password is written — an account without a usable credential stays unable to log in. The direction is read from `/etc/shadow` when the confirmation opens and again immediately before acting; where it cannot be read, neither direction is offered rather than the forward one being assumed. |
...
| `ssh.uninstall` | `run <id> name=value` | **yes** | Removes the OpenSSH server. **The one operation in this tool with no route back:** run over SSH it ends the session mid-removal, and reinstalling needs the network path that just closed — recovery is the provider's console. Absent by design until it was asked for; the reasoning behind that absence has not changed, which is why it carries the strongest confirmation the interface has. Shares a row with `ssh.install` in the TUI, and is only offered once the host is measured as having a server installed. |
| `ssh.harden` | `run ssh.harden` | yes | Disables root login, password authentication, agent and X11 forwarding, tunnelling and user environments; limits authentication attempts and the login grace period; enables verbose logging. Refuses unless some account other than root holds an authorised key — root does not count, since this task disables it. |
| `ssh.harden-strict` | `run ssh.harden-strict` | yes | Restricts key exchange, cipher, MAC and host key algorithms to a modern set, requires 3072-bit RSA keys, and disables TCP forwarding. Refuses unless some account the configuration still admits holds an authorised key. |
...
| `firewall.enable` | `run <id> name=value` | yes | Closes every inbound port except the one named, plus established connections and loopback. Nothing is opened: the port parameter exists because a default-deny policy drops the session running the task unless that session's port is admitted in the same ruleset. The port defaults to whatever `sshd -T` reports this host is listening on, in both interfaces — a compiled-in 22 would close the session's own port on a host whose SSH has moved. An explicit value always wins over the one read from the host. The ruleset is saved where the boot replays it; on a host with no service manager it reports that it was saved and will not be replayed. |
...
| `firewall.manage-ports` | `run <id> name=value` | yes | Declares which ports are admitted inbound, as `ports="22/tcp 443/tcp"`: anything listed is opened, anything the host admits and the list omits is closed. Ports are opened before any is closed, so a set that moves a service between ports does not drop the session in between. `ports` defaults to what the host currently admits, in both interfaces — so an invocation naming nothing is a no-op rather than "close everything", which the empty set does mean when written out explicitly. Lockout, because a port left out of the set may be the one carrying the session. Only ports named directly are closed: firewalld admits SSH on a stock RHEL host as the *service* `ssh`, where `--remove-port` succeeds and changes nothing, so those are reported as still open rather than claimed closed. A second parameter `ports_were` carries what was on screen when the operator decided; a port the host gained since is reported and left alone. Empty from the CLI, where a declared set is taken to mean exactly itself. Saved across reboots on the same terms as `firewall.enable`. Refuses when nothing is being filtered, since against no default-deny policy every port is already reachable — the error names `firewall.enable` as the missing step. |
...
| Task id | Invocation | Lockout | Summary |
|---------|-----------|---------|---------|
| `docker.install` | `run <id>` | no | Installs the container engine for the whole machine and starts it. Where Docker publishes a repository for the distribution (Debian, Ubuntu, RHEL and its rebuilds), registers it first — refusing if its signing key does not match the fingerprint this build carries — and installs the five packages upstream's page lists in one transaction. Elsewhere (Arch, openSUSE, Alpine), installs the distribution's own package, since Docker publishes no repository for those. |
| `docker.uninstall` | `run <id> name=value` | no | Stops the engine, disables it at boot and removes it. Images, volumes and containers under `/var/lib/docker` are left alone. Accounts running a rootless engine keep theirs. |
| `docker.rootless` | `run <id> name=value` | no | Sets the engine up under one account rather than as root, with lingering enabled. Refuses a host with no engine installed (`docker.install` first), and an account with no subordinate id range or no reachable user session. On Arch, where no official package ships `dockerd-rootless-setuptool.sh`, fetches it from `get.docker.com/rootless` — which publishes no digest to verify it against, stated as a consequence before the task runs. |
